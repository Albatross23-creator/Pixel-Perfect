...
def load_model():
    ...
    if not os.path.exists(MODEL_NAME):
        ...
        try:
            ...
            st.error(
                f"Failed to download model: {e}. Please ensure you have an internet connection and the URL is correct.",
                icon="❌")
            return None
    else:
    ...
    try:
        ...
        st.error(f"Error loading model weights: {e}. Please check the model file integrity and compatibility.",
                 icon="❌")
        return None
...
def enhance_single_image_helper(image_pil, enhancement_strength, original_filename):
    ...
    with st.spinner(f"🚀 Enhancing {original_filename}..."):
        try:
            ...
            st.error(
                f"GPU ran out of memory for {original_filename}. Try a smaller image or consider setting `tile` in `RealESRGANer`.",
                icon="🚨")
            return None
        except Exception as e:
...
def remove_background_helper(image_pil):
    ...
    try:
        ...
        st.error(
            "The 'rembg' library is not installed. Please install it using: `pip install rembg` and `pip install onnxruntime`",
            icon="❌")
        return None
    except Exception as e:
...
def _add_to_history(image_pil_to_add, specific_caption_for_this_step=None):
    ...
    if not st.session_state['image_history']:
        if st.session_state['original_uploaded_image_pil'] is not None:
            st.session_state['image_history'].append(
                (st.session_state['original_uploaded_image_pil'].copy(), "Original Image"))
    ...
    caption_to_store = specific_caption_for_this_step if len(
        st.session_state['image_history']) == 1 and specific_caption_for_this_step is not None else "Processed Image"
    ...
    st.session_state['current_edited_image_pil'] = st.session_state['image_history'][st.session_state['history_index']][
        0]
    st.session_state['processed_image_for_download'] = st.session_state['current_edited_image_pil']
    st.session_state['current_display_caption'] = st.session_state['image_history'][st.session_state['history_index']][
        1]
...
if st.session_state['original_uploaded_image_pil'] is not None:
    ...
    with col_redo:
        if st.button(
                "Redo Last Action",
                key="redo_button",
                disabled=(st.session_state['history_index'] >= len(st.session_state['image_history']) - 1 or not
                st.session_state['image_history']),
                use_container_width=True
        ):
    ...
    if st.session_state['original_uploaded_image_pil'] is not None:
        if st.session_state['current_edited_image_pil'] is None or \
                (st.session_state['current_edited_image_pil'] is not None and
                 st.session_state['current_edited_image_pil'].tobytes() == st.session_state[
                     'original_uploaded_image_pil'].tobytes() and
...
elif st.session_state['original_uploaded_image_pil'] is not None and st.session_state[
    'current_edited_image_pil'] is None:
...
if st.session_state['original_uploaded_image_pil'] is not None:
    with col1_orig:
        ...
        st.image(st.session_state['original_uploaded_image_pil'],
                 caption=f"Name: {st.session_state['current_image_name']}", use_container_width=True)
        original_image_size_kb = len(
            get_image_bytes(st.session_state['original_uploaded_image_pil'], "image/png")) / 1024
        st.metric(label="Dimensions",
                  value=f"{st.session_state['original_uploaded_image_pil'].width}x{st.session_state['original_uploaded_image_pil'].height} px")
        st.metric(label="Size", value=f"{original_image_size_kb:.2f} KB")

    with col2_processed:
        # Dynamic display for processed image or ELA result
        if st.session_state.get('display_ela_in_processed_area', False) and st.session_state.get(
                'ela_result_image') is not None:
            ...
            st.image(st.session_state['current_edited_image_pil'], caption=st.session_state['current_display_caption'],
                     use_container_width=True)
            current_edited_size_kb = len(
                get_image_bytes(st.session_state['current_edited_image_pil'], "image/png")) / 1024
            st.metric(label="Dimensions",
                      value=f"{st.session_state['current_edited_image_pil'].width}x{st.session_state['current_edited_image_pil'].height} px")
    ...
    st.info(
        "⬆️ Please upload an image or multiple images in the sidebar to begin. Then select a tab to apply operations.",
        icon="⬆️")
    st.image(
        "https://assets-global.website-files.com/5f72674e1d13755b41a3884b/60b9432f22b07e86e74b338d_Streamlit-logo.png",
        width=300)
...
with tab1:
    ...
    if st.button("🚀 Enhance Image(s)", use_container_width=True, key="enhance_button"):
        if st.session_state['original_uploaded_image_pil'] is not None:  # Check if a single image is uploaded
            image_to_enhance = st.session_state['current_edited_image_pil'] if st.session_state[
                                                                                   'current_edited_image_pil'] is not None else \
            st.session_state['original_uploaded_image_pil']
            ...
            for i, uploaded_file in enumerate(st.session_state['uploaded_files_list']):
                my_bar.progress((i + 1) / len(st.session_state['uploaded_files_list']),
                                text=f"Enhancing {uploaded_file.name}...")
                image_to_process = Image.open(uploaded_file).convert('RGB')
                enhanced_img = enhance_single_image_helper(image_to_process, enhancement_strength, uploaded_file.name)
                if enhanced_img:
                    base_name = os.path.splitext(uploaded_file.name)[0]
                    processed_files.append(
                        (enhanced_img, get_image_bytes(enhanced_img, "image/png"), f"enhanced_{base_name}.png"))
            my_bar.empty()
...
with tab2:
    ...
    if st.button("🗜️ Compress Image(s)", use_container_width=True, key="compress_quality_button"):
        if st.session_state['original_uploaded_image_pil'] is not None:
            image_to_compress = st.session_state['current_edited_image_pil'] if st.session_state[
                                                                                    'current_edited_image_pil'] is not None else \
            st.session_state['original_uploaded_image_pil']
            ...
            for i, uploaded_file in enumerate(st.session_state['uploaded_files_list']):
                my_bar.progress((i + 1) / len(st.session_state['uploaded_files_list']),
                                text=f"Compressing {uploaded_file.name}...")
                image_to_process = Image.open(uploaded_file).convert('RGB')
                compressed_img = compress_image_by_quality_func(image_to_process, compression_quality)
                if compressed_img:
                    base_name = os.path.splitext(uploaded_file.name)[0]
                    processed_files.append((compressed_img,
                                            get_image_bytes(compressed_img, "image/jpeg", quality=compression_quality),
                                            f"compressed_q{compression_quality}_{base_name}.jpg"))
            my_bar.empty()
...
with tab3:  # Tab name changed to "Rotate & Resize"
    ...
    if st.session_state['original_uploaded_image_pil'] is not None:
        ...
        rotation_angle = st.slider("Rotation Angle (degrees)", -180, 180, 0, 5,
                                   help="Rotate the image clockwise (positive) or counter-clockwise (negative).",
                                   key="rotation_angle_slider")

        if st.button("🔄 Apply Rotation", use_container_width=True, key="apply_rotation_button"):
            image_to_rotate = st.session_state['current_edited_image_pil'] if st.session_state[
                                                                                  'current_edited_image_pil'] is not None else \
            st.session_state['original_uploaded_image_pil']
        ...
        image_to_resize_source_percent = st.session_state['current_edited_image_pil'] if st.session_state[
                                                                                             'current_edited_image_pil'] is not None else \
        st.session_state['original_uploaded_image_pil']
        ...
        scale_percentage = st.slider("Scale Percentage", 10, 200, 100, 5,
                                     help="Set the desired scale percentage (e.g., 50% for half size, 200% for double size).",
                                     key="scale_percentage_slider")
        ...
        if st.button("📏 Apply Percentage Resize", use_container_width=True, key="apply_percentage_resize_button"):
            ...
            try:
                ...
                st.success(
                    f"Image resized to {new_width}x{new_height} successfully! Check the 'Processed Image' above.",
                    icon="✅")
                st.rerun()
            except Exception as e:
        ...
        image_to_resize_custom = st.session_state['current_edited_image_pil'] if st.session_state[
                                                                                     'current_edited_image_pil'] is not None else \
        st.session_state['original_uploaded_image_pil']
        ...
        def update_custom_height_from_width():
            if st.session_state.get('lock_aspect_ratio_custom_checkbox', False):
                if current_height_custom != 0:
                    ...
                    st.session_state.custom_resize_new_height = int(
                        st.session_state.custom_resize_new_width / current_ratio)
        ...
        def update_custom_width_from_height():
            if st.session_state.get('lock_aspect_ratio_custom_checkbox', False):
                if current_width_custom != 0:
                    current_ratio = current_width_custom / current_height_custom
                    st.session_state.custom_resize_new_width = int(
                        st.session_state.custom_resize_new_height * current_ratio)
        ...
        if st.button("📏 Apply Custom Resize", use_container_width=True, key="apply_custom_resize_button"):
            try:
                ...
                st.success(
                    f"Image resized to {final_width}x{final_height} successfully! Check the 'Processed Image' above.",
                    icon="✅")
                st.rerun()
            except Exception as e:
...
with tab4:
    ...
    if st.session_state['original_uploaded_image_pil'] is not None:
        ...
        image_to_adjust = st.session_state['current_edited_image_pil'] if st.session_state[
                                                                              'current_edited_image_pil'] is not None else \
        st.session_state['original_uploaded_image_pil']
        ...
        with col_adj1:
            brightness = st.slider("Brightness", 0.0, 2.0, 1.0, 0.05, help="Adjusts the overall lightness/darkness.",
                                   key="adj_brightness")
            contrast = st.slider("Contrast", 0.0, 2.0, 1.0, 0.05,
                                 help="Adjusts the difference between light and dark areas.", key="adj_contrast")
        with col_adj2:
            saturation = st.slider("Saturation", 0.0, 2.0, 1.0, 0.05, help="Adjusts the intensity of colors.",
                                   key="adj_saturation")
        ...
        with col_rgb1:
            red_gain = st.slider("Red Channel", 0.0, 2.0, 1.0, 0.05, help="Adjusts the intensity of the Red channel.",
                                 key="adj_red_gain")
        with col_rgb2:
            green_gain = st.slider("Green Channel", 0.0, 2.0, 1.0, 0.05,
                                   help="Adjusts the intensity of the Green channel.", key="adj_green_gain")
        with col_rgb3:
            blue_gain = st.slider("Blue Channel", 0.0, 2.0, 1.0, 0.05,
                                  help="Adjusts the intensity of the Blue channel.", key="adj_blue_gain")

        st.markdown("---")
        st.markdown("### Advanced Adjustments (Requires OpenCV)")
        hue_shift = st.slider("Hue Shift", -180, 180, 0, 10,
                              help="Shifts the hue of the colors in the image. Range is -180 to 180, corresponding to -90 to 90 degrees in OpenCV's 0-179 hue scale.",
                              key="adj_hue")
...
with tab5:
    ...
    if st.session_state['original_uploaded_image_pil'] is not None:
        ...
        image_to_denoise = st.session_state['current_edited_image_pil'] if st.session_state[
                                                                               'current_edited_image_pil'] is not None else \
        st.session_state['original_uploaded_image_pil']

        h_denoise = st.slider("Denoising Strength (h)", 0, 50, 10,
                              help="Parameter controlling filter strength. Higher value removes more noise but may blur details.")
        h_color_denoise = st.slider("Color Denoising Strength (hColor)", 0, 50, 10,
                                    help="Parameter controlling filter strength for color components. Higher value removes more color noise.")
        template_window_size = st.slider("Template Window Size", 1, 21, 7, 2,
                                         help="Size of the pixel neighborhood used to calculate weighted average. Must be odd.")
        search_window_size = st.slider("Search Window Size", 1, 21, 21, 2,
                                       help="Size of the window that is used to compute weighted average for given pixel. Must be odd.")
...
with tab6:
    ...
    st.info("If you encounter errors, ensure `rembg` is installed: `pip install rembg` and `pip install onnxruntime`",
            icon="💡")

    if st.session_state['original_uploaded_image_pil'] is not None:
        st.info("Background removal is only available for single image processing.", icon="ℹ️")

        image_to_process_bg = st.session_state['current_edited_image_pil'] if st.session_state[
                                                                                  'current_edited_image_pil'] is not None else \
        st.session_state['original_uploaded_image_pil']
...
with tab7:
    ...
    st.write(
        "ELA highlights areas in a JPEG image that have been compressed at different rates, which can indicate tampering. Modified regions often have a higher ELA signal.")
    st.warning(
        "ELA is most effective on JPEG images. If your uploaded image is not JPEG, it will be converted to JPEG (quality 95) for analysis. The result is grayscale where brighter areas indicate higher error levels/potential manipulation.",
        icon="💡")

    if st.session_state['original_uploaded_image_pil'] is not None:
        st.info("ELA is only available for single image processing.", icon="ℹ️")

        image_for_ela = st.session_state['current_edited_image_pil'] if st.session_state[
                                                                            'current_edited_image_pil'] is not None else \
        st.session_state['original_uploaded_image_pil']
        ...
        if st.button("🕵️ Generate ELA", use_container_width=True, key="generate_ela_button"):
            with st.spinner("Generating ELA image..."):
                try:
                    ...
                    st.warning(
                        "ELA works best on JPEG images. If your image is PNG, try converting it to JPEG first using the Compression tab.",
                        icon="ℹ️")
    else:
